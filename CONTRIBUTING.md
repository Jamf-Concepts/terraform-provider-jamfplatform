...
5. **Build the package** at `internal/resources/pro/<domain>/<resource>/` per the [file conventions](STYLE_GUIDE.md#resource-package-file-conventions). Mirror `internal/resources/blueprints/blueprint/` for complex shapes. Resource Configure **must** call `pd.GetJamfProVersion(ctx)` and `helpers.RequireMinJamfProVersion` when `minJamfProVersion != ""`. Credentials are the same `JAMFPLATFORM_*` set used by Platform Services resources — there is no separate Pro credentials gate. **`crud.go` must carry the SDK-endpoints annotation block** at the top of the file with `Status: current. Last reviewed YYYY-MM-DD.` See [STYLE_GUIDE.md §Endpoint adoption & migration policy](STYLE_GUIDE.md#endpoint-adoption--migration-policy).
