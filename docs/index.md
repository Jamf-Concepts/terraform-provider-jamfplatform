...
This does not change any Jamf Pro setting. To configure the impact alerts Jamf Pro shows in its own web interface, use `jamfplatform_pro_impact_alert_notification_settings`. Can also be set via the JAMFPLATFORM_IMPACT_ALERTS environment variable. See the [Impact alerts guide](../guides/impact-alerts) for how to read the figures and how to consume them in CI.
