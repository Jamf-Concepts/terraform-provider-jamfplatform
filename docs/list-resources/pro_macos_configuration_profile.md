...
subcategory: ""
description: |-
  Lists macOS configuration profiles in the tenant. Supply an optional case-insensitive name_substring filter. Filtering happens in the provider, so every profile is fetched before the filter runs. List entries return identity only; use the jamfplatform_pro_macos_configuration_profile data source for per-profile detail.
