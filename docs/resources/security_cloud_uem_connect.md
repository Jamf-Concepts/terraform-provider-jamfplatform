...
- `sync_refresh_interval_minutes` (Number) **"Sync refresh interval"** in the Jamf Security Cloud admin UI, in minutes. The admin UI offers a fixed set of intervals; any value of 1 or more is accepted here, but prefer one the admin UI also offers so the two agree. Defaults to 1440 (24 hours).
