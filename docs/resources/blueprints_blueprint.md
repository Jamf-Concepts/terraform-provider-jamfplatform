...
- `raw_component` (Attributes Set, Deprecated) Raw component configuration using key-value pairs. Use it for a component this resource has no attribute for, or to manage one without the checks its own attribute applies. Do not declare a component here that another component attribute set alongside it already manages: the platform stores both copies and only one of them can be represented in state, so the other would reach devices without ever appearing in a plan. Keep one of the two. (see [below for nested schema](#nestedatt--raw_component))
