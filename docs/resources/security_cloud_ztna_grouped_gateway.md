...
description: |-
  ...
  Every member must be one of your own dedicated gateways (Jamf Security Cloud's shared gateways are refused), and all members must be the same form, either all IPsec or all internet. Deleting a member gateway while it is still in a group is refused.
...
Members must be your own dedicated gateways, all of the same form, so mixing an IPsec gateway with an internet one is refused, and so is naming one of Jamf Security Cloud's shared gateways.
