...
- `regeneration_trigger` (String) Rotation trigger. Set to any free-text value (e.g. a version label or timestamp). Changing this value on a subsequent apply forces the provider to call the failover regenerate endpoint and produces a new `failover_url`.
