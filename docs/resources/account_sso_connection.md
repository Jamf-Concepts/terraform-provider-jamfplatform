...
- `generic_oidc` (Attributes) Settings for a connection to any OpenID Connect provider Jamf has no purpose-built integration with. Required when `connection_type` is `generic_oidc`, and refused otherwise.
...
- `managed_account_id` (String) Set this only when the environments belong to an account you manage on someone else's behalf as a Jamf partner. Nothing Jamf returns reveals it.
...
- `user_info_endpoint` (String) Address profile details are read from. Leave it out when your provider returns everything Jamf needs in the identity token itself.
