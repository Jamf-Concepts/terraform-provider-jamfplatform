...
**Creating** an IPsec gateway waits for nothing, because the tunnel cannot be up yet: its other end is built from values this resource returns, so the concentrator, its NAT rules and its firewall openings all come afterwards. Expect `PENDING` in state on the first apply, settling to `DOWN` until the tunnel is established and to `UP` once it is — a later refresh records it. **Updating** one does wait, since by then the tunnel may already be up; if it is down, the apply gives up after the gateway has reported `DOWN` for a minute and warns rather than holding for the whole budget.
