...
- `server_token_wo_version` (Number) Rotation trigger for the `WriteOnly` `server_token`. Bump this integer (any change) to force an update that re-sends the current `server_token` to Jamf Pro. Initial create should set `server_token_wo_version = 1`. Required because `server_token` itself is Required — keeping the companion Required keeps the rotation signal explicit in config. Bumping this value triggers a fresh Apple DEP sync; the update operation blocks until Jamf Pro reports the sync as `SUCCESSFUL` (subject to `timeouts.update`, default 5 minutes).
