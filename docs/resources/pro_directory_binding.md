...
- `active_directory` (Attributes) Active Directory–specific configuration. May only be set when `type = "Active Directory"`; setting it for any other type is a plan-time error. (see [below for nested schema](#nestedatt--active_directory))
- `admitmac` (Attributes) ADmitMac–specific configuration. May only be set when `type = "ADmitMac"`; setting it for any other type is a plan-time error. (see [below for nested schema](#nestedatt--admitmac))
- `centrify` (Attributes) Centrify–specific configuration. May only be set when `type = "Centrify"`; setting it for any other type is a plan-time error. (see [below for nested schema](#nestedatt--centrify))
...
- `open_directory` (Attributes) Open Directory–specific configuration. May only be set when `type = "Open Directory"`; setting it for any other type is a plan-time error. (see [below for nested schema](#nestedatt--open_directory))
