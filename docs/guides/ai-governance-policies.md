...
Terraform refuses that rather than applying it. Where a version is on record, every update is made conditional on the policy still being at the version last read; if anything wrote to the policy in between, Jamf rejects the update, **nothing is changed**, and the apply reports *AI policy is not at the version Terraform last read*. Clearing it takes a **new plan**. The refresh picks up the current policy and that plan reports what this configuration would alter, including whatever the other change introduced — so the overwrite happens only if you approve it. A saved plan carries the version it was made against, so re-applying the same plan file resends the stale value and is refused the same way however often you retry it.

The check needs a version to compare against, and **a policy Jamf created before it gained that counter reports none**. Those are updated unconditionally, exactly as they were before. So is a policy whose state was written by an earlier release of this provider and applied with `-refresh=false`, because nothing has recorded a version for it yet. Every apply that writes the policy records one, so the apply after that is conditional even without a refresh. Nothing reports either case; both behave as the resource did before the check existed.
