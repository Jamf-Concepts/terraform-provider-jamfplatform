...
// (the receiver is r.client / d.client across the constructs in this package).
var clientCallRe = regexp.MustCompile(`\bclient\.([A-Za-z0-9]+)\(`)

// sdkCallsIn reads a construct source file and returns the set of SDK client
// method names it calls, filtered to those known to the pro privilege registry.
...
// assertMatch fails if the construct's SDK calls and its declared method list
...
func TestResourcePrivileges_Rendered(t *testing.T) {
	if !strings.Contains(resourcePrivileges, "create:pro:api-roles") {
		t.Fatalf("resourcePrivileges did not render the api-roles privileges:\n%s", resourcePrivileges)
	}
}

// --- data source (data_source.go) ---

// TestDataSourceSDKMethods_KnownToSDK fails on SDK drift.
...
// TestDataSourceSDKMethods_MatchCalls keeps dataSourceSDKMethods in sync with
...
func TestDataSourcePrivileges_Rendered(t *testing.T) {
	if !strings.Contains(dataSourcePrivileges, "read:pro:api-roles") {
		t.Fatalf("dataSourcePrivileges did not render the api-roles privileges:\n%s", dataSourcePrivileges)
	}
}

// --- plural data source (datasource_plural.go) ---

// TestPluralDataSourceSDKMethods_KnownToSDK fails on SDK drift.
...
// TestPluralDataSourceSDKMethods_MatchCalls keeps pluralDataSourceSDKMethods in
// sync with the client.<Method> calls in datasource_plural.go.
...
func TestPluralDataSourcePrivileges_Rendered(t *testing.T) {
	if !strings.Contains(pluralDataSourcePrivileges, "read:pro:api-roles") {
		t.Fatalf("pluralDataSourcePrivileges did not render the api-roles privileges:\n%s", pluralDataSourcePrivileges)
	}
}

// --- list resource (list_resource.go) ---

// TestListResourceSDKMethods_KnownToSDK fails on SDK drift.
...
// TestListResourceSDKMethods_MatchCalls keeps listResourceSDKMethods in sync
...
func TestListResourcePrivileges_Rendered(t *testing.T) {
	if !strings.Contains(listResourcePrivileges, "read:pro:api-roles") {
		t.Fatalf("listResourcePrivileges did not render the api-roles privileges:\n%s", listResourcePrivileges)
	}
}
