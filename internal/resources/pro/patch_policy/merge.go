...
// unmanagedPatchPolicyScopeCategories lists the categories of a live
// patch-policy scope that the declared plan leaves unmanaged. plan must be
// non-nil; server is the hydrate-all flatten of the live object's scope (nil
// means an empty live scope — nothing to report).
func unmanagedPatchPolicyScopeCategories(plan, server *PatchPolicyScopeModel) []string {
	...
	out = scope.AppendUnmanagedCategory(out, "limitations.ibeacon_ids", pl.IbeaconIDs, sl.IbeaconIDs)
	...
	out = scope.AppendUnmanagedCategory(out, "exclusions.ibeacon_ids", pe.IbeaconIDs, se.IbeaconIDs)
	return out
}
