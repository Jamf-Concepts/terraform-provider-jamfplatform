...
func TestUnmanagedPatchPolicyScopeCategories(t *testing.T) {
	plan := &PatchPolicyScopeModel{
		Targets: &PatchPolicyScopeTargetsModel{
			ComputerGroupIDs: idSet(t, "10"), // declared → not reported
		},
	}
	server := &PatchPolicyScopeModel{
		Targets: &PatchPolicyScopeTargetsModel{
	...
	got := unmanagedPatchPolicyScopeCategories(plan, server)
	want := map[string]bool{
		"targets.all_computers":   true,
		"targets.building_ids":    true,
		"limitations.ibeacon_ids": true,
	}
	if len(got) != len(want) {
	...
	if unmanagedPatchPolicyScopeCategories(nil, server) != nil {
		t.Error("nil plan must report nothing")
	}
	if unmanagedPatchPolicyScopeCategories(plan, nil) != nil {
