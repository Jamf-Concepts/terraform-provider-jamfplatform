...
func (r *PackageListResource) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "Searches Jamf Pro packages via the V1 /packages endpoint's RSQL filter. The supported selectors are a server-enforced whitelist — see `selector` attribute for the allowed values.",
