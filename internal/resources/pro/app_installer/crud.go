...
//   pro.ListAppInstallerTitlesV1       (app_title_name → id, plan-time + apply)
...
// id back to the name (the GET returns only the id). Both name lookups go through
// Jamf Pro's own RSQL filter and then decide the match on byte equality, because
// the filter is a case-insensitive glob — see name_lookup.go. selected_version is
