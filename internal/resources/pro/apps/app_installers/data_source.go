...
// Package app_installers implements the read-only jamfplatform_pro_app_installers
// plural data source, which returns every App Installer deployment for the
// tenant in its expanded form (nested app / site / category / smart-group refs
// plus per-deployment computer status counts). Use jamfplatform_pro_app_installer
// (singular) for a flat by-id/by-name lookup, or to manage a deployment.
