...
// Package macos_configuration_profile implements the
// jamfplatform_pro_macos_configuration_profile resource, data source, and
// list resource backed by the Jamf ProClassic osxconfigurationprofiles API.
...
func maskTopLevel(p map[string]any) map[string]any {
	...
	for k, v := range p {
		...
		trimmed := trimAny(v)
		if isEmpty(trimmed) {
			// Drop empty-string / nil values so they compare equal to absent.
			// Jamf Pro replaces empty strings with a server default (e.g.
			// `PayloadOrganization=""` → tenant org name), which is the same
			// semantic outcome as the user omitting the key.
			continue
		}
		out[k] = trimmed
	}
...
// author anything" — empty string, nil, or empty collection. Keys with
// such values are dropped before intersection compare so a user-authored
// empty matches a server-stamped default (both represent "no real value
// supplied"). Non-empty values fall through to the lenient compare.
...
// values.
//
// Trade-off documented in the resource's MarkdownDescription: a user who
// edits only a key Jamf Pro itself derives (e.g. tunes `PayloadOrganization`
// from empty to a value, then back) won't see a diff — the server will
// substitute its own value, so flagging that as drift would produce a
// persistent diff loop.
...
// state.General.UUID — on the Classic API the server's <general><uuid>
// matches the top-level PayloadUUID, and PayloadIdentifier is also set
// to that same lowercase UUID for profiles minted by Jamf Pro).
//
// Returns the modified plist serialised back to XML in the canonical
// howett.net/plist XMLFormat. Equivalent in spirit to jamfpro-cli
// `profileconvert.InjectIdentifiers`. Without this step, every PUT
...
// injectTopLevelIdentifiers is the existingPayload-based wrapper retained
// for unit-test coverage of the parsing path. CRUD callers use
// injectTopLevelIdentifierValues directly so they can pass server-canonical
// identifiers sourced from state.General.UUID (which carries the trustworthy
// value; the bytes in state.General.Payloads are user-authored).
...
// CDATA). The SDK already decodes the entity references when populating
// OsXConfigurationProfileGeneral.Payloads, so callers receive the raw plist.
// This helper exists for tests that work directly with captured wire XML.
func extractServerPayloadFromGeneral(wireXML []byte) ([]byte, error) {
	...
	// XML-entity-encoded form. The classic API uses numeric entity refs
	// (&#34;, &#10;) as well as named ones (&lt;, &gt;, &amp;), so reach
	// for html.UnescapeString rather than rolling our own table.
