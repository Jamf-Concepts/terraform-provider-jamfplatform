...
func (r *ListResource) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		Description: "Lists macOS configuration profiles. Classic has no RSQL — supply an optional case-insensitive `name_substring` filter applied client-side. List entries surface as identity-only; full profile detail requires a per-profile read.",
