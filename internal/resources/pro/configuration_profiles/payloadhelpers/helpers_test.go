...
// SDK workaround stale: when Jamf Pro's "double-decode bug" is fixed, the
// SDK's PayloadsXMLText.MarshalXML pre-escape leaves an extra entity layer
// in <string> values on read — e.g. `&#34;` literal where the user wrote
// `"`. PayloadsSemanticallyEqual must tolerate this and CanonicalisePlistXML
// must collapse it.
func TestPayloadsSemanticallyEqual_EntityEncodedQuotesTolerated(t *testing.T) {
	user := []byte(`<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict><key>PayloadType</key><string>Configuration</string><key>PayloadUUID</key><string>U</string><key>PayloadIdentifier</key><string>I</string><key>PayloadDisplayName</key><string>N</string><key>PayloadVersion</key><integer>1</integer><key>PayloadContent</key><array><dict><key>PayloadType</key><string>com.apple.TCC.configuration-profile-policy</string><key>CodeRequirement</key><string>identifier "com.foo" and OU = "ABC"</string></dict></array></dict></plist>`)
	// Server bytes carry an extra entity layer in the string value
	// (post-fix Jamf behaviour as of 2026-05-27).
	server := []byte(`<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict><key>PayloadType</key><string>Configuration</string><key>PayloadUUID</key><string>U</string><key>PayloadIdentifier</key><string>I</string><key>PayloadDisplayName</key><string>N</string><key>PayloadVersion</key><integer>1</integer><key>PayloadContent</key><array><dict><key>PayloadType</key><string>com.apple.TCC.configuration-profile-policy</string><key>CodeRequirement</key><string>identifier &amp;#34;com.foo&amp;#34; and OU = &amp;#34;ABC&amp;#34;</string></dict></array></dict></plist>`)
	eq, err := PayloadsSemanticallyEqual(user, server)
	if err != nil {
		t.Fatalf("compare: %v", err)
	}
	if !eq {
		t.Fatal("expected equal across extra entity-encode layer")
	}
}

func TestCanonicalisePlistXML_CollapsesExtraEntityLayer(t *testing.T) {
	server := []byte(`<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict><key>PayloadContent</key><array><dict><key>CodeRequirement</key><string>identifier &amp;#34;com.foo&amp;#34;</string></dict></array></dict></plist>`)
	out := CanonicalisePlistXML(server)
	if strings.Contains(string(out), "&amp;#34;") {
		t.Fatalf("expected entity layer collapsed; got:\n%s", string(out))
	}
	if !strings.Contains(string(out), "&#34;") {
		t.Fatalf("expected single-layer &#34; in canonical output; got:\n%s", string(out))
	}
}

func TestDecodeXMLEntitiesOnce_PreservesAmbiguousValues(t *testing.T) {
	for _, tc := range []struct {
		in, want string
	}{
		{`identifier "X"`, `identifier "X"`}, // no entities
		{`&#34;X&#34;`, `"X"`},               // numeric -> quote
		{`https://example.com?a=1&amp;b=2`, `https://example.com?a=1&b=2`},
		{`literal &amp;copy; preserved`, `literal &copy; preserved`}, // one-pass decode
		{`literal & alone`, `literal & alone`},                       // bare & -> bail
		{`has <bracket>`, `has <bracket>`},                           // bare < -> bail
		{`unknown &nope; entity`, `unknown &nope; entity`},           // unknown entity -> bail
	} {
		if got := decodeXMLEntitiesOnce(tc.in); got != tc.want {
			t.Errorf("decodeXMLEntitiesOnce(%q) = %q, want %q", tc.in, got, tc.want)
		}
	}
}
