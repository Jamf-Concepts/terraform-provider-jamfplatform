...
func (d *SmtpServerDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Read the current Jamf Pro SMTP Server settings (Settings → System → SMTP Server). Singleton — one record per tenant. WriteOnly secrets are never returned.",
