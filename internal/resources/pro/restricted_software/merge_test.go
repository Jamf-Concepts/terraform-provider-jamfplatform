...
func TestUnmanagedRestrictedSoftwareScopeCategories(t *testing.T) {
	plan := &RestrictedSoftwareScopeModel{
		Targets: &RestrictedSoftwareScopeTargetsModel{
			ComputerGroupIDs: strSet(t, "10"), // declared → not reported
		},
	}
	server := &RestrictedSoftwareScopeModel{
		Targets: &RestrictedSoftwareScopeTargetsModel{
			...
			ComputerGroupIDs: strSet(t, "99"),
			DepartmentIDs:    strSet(t, "3"),
			BuildingIDs:      scope.EmptyStringSet(), // empty live → not reported
		},
		Exclusions: &RestrictedSoftwareScopeExclusionsModel{
			DirectoryServiceOrLocalUserNames: strSet(t, "alice"),
		},
	}
	got := unmanagedRestrictedSoftwareScopeCategories(plan, server)
	want := map[string]bool{
		"targets.all_computers":                            true,
		"targets.department_ids":                           true,
		"exclusions.directory_service_or_local_user_names": true,
	}
	if len(got) != len(want) {
	...
	if unmanagedRestrictedSoftwareScopeCategories(nil, server) != nil {
		t.Error("nil plan must report nothing")
	}
	if unmanagedRestrictedSoftwareScopeCategories(plan, nil) != nil {
