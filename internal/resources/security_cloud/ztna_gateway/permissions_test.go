...
// TestEnumValuesComeFromSDK guards the single-source-of-truth claim: the
// validators and the documented value lists are both generated from these
// slices, so a silently empty one would take both down at once and the schema
// would accept anything.
func TestEnumValuesComeFromSDK(t *testing.T) {
	for name, values := range map[string][]string{
		"datacenter":  datacenterValues(),
		"keyExchange": keyExchangeValues(),
		"encryption":  encryptionValues(),
		"integrity":   integrityValues(),
		"dhGroups":    dhGroupValues(),
		"vendor":      vendorValues(),
	} {
		if len(values) == 0 {
...
// TestVendorValuesAreCaseSensitive pins the one enum whose casing is easy to get
// wrong and impossible to diagnose from the server's reply: `strongswan` is
// refused, `strongSwan` accepted (wire-probed 2026-08-27).
func TestVendorValuesAreCaseSensitive(t *testing.T) {
	var found bool
