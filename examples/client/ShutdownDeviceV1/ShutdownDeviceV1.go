...
func main() {
	...
	commands, err := apiClient.ShutdownDeviceV1(context.Background(), deviceID)
	if err != nil {
		log.Fatalf("Error shutting down device %s: %v", deviceID, err)
	}

	fmt.Printf("Shutdown requested for device %s\n", deviceID)
