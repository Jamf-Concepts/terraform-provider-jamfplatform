...
func main() {
	...
	commands, err := apiClient.UnmanageDeviceV1(context.Background(), deviceID)
	if err != nil {
		log.Fatalf("Error unmanaging device %s: %v", deviceID, err)
	}

	fmt.Printf("Unmanage requested for device %s\n", deviceID)
