...
func main() {
	...
	// Undeploy blueprint by ID
	fmt.Printf("Undeploying blueprint: %s\n", blueprintID)
	if err := apiClient.UndeployBlueprintV1(context.Background(), blueprintID); err != nil {
		log.Fatalf("Error undeploying blueprint %s: %v", blueprintID, err)
	}
	fmt.Println("Undeployment started successfully (202 Accepted)")
}
